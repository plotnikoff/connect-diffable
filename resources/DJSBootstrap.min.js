...
DJSBootstrap.checkStorage=function(a,b){if(!window.localStorage||!window.localStorage.diffable===undefined)DJSBootstrap.loadVersion(a);else{var d=JSON.parse(localStorage.diffable);if(d[a]&&d[a].v===b){window.execScript?window.execScript(d[a].c):window.eval.call(window,d[a].c);console.log("fromlocalstorage")}else DJSBootstrap.loadVersion(a)}};
